...
from ..common.models.base import Budget, EvaluationVerdict, TaskPhase, TraceEntry
...
from ..common.models.task_state import StageStatus, TaskState
...
class Orchestrator:
    ...
    async def _gather_evidence(self, task_state: TaskState, phase: TaskPhase) -> dict[str, Any]:
        ...
        if phase == TaskPhase.PAPER_PARSING:
            candidates = task_state.metadata.get("paper_candidates", [])
            if candidates:
                evidence["selected_paper"] = candidates[0] if candidates else None
