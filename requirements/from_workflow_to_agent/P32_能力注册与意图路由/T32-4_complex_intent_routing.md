...
| T32-4.8 | 澄清与低置信度策略 | 统一处理不确定、缺参和冲突意图 | T32-4.5、T32-4.7 | 待实现 |
| T32-4.9 | Workflow 意图映射 | 将组合意图映射到受控 Workflow | T32-4.7、P33 | 待实现 |
| T32-4.10 | 安全与执行边界 | 限制能力集合、参数和调用权限 | T32-4.5 | 待实现 |
| T32-4.11 | 评测与可观测性 | 验证准确性、负向行为和决策原因 | T32-4.7、T32-4.8 | 待实现 |
...
- [ ] LLM 输出永远经过 JSON Schema 校验；
- [ ] 未注册能力永远不能被执行；
- [ ] 缺少参数时只返回澄清，不调用 Adapter；
- [ ] 低置信度时只返回澄清，不调用 Adapter；
- [ ] 确定性路由优先于 LLM 路由；
- [ ] 上下文引用不能绕过 Session/Task 校验；
- [ ] LLM 不能直接创建 Tool、Workflow 或命令；
- [ ] 组合意图只能映射到已注册 Workflow；
- [ ] 正向、负向、多轮和安全评测均有自动化测试；
- [ ] 路由决策具备可追踪原因和脱敏日志。
